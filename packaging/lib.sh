...
# manifest, the spec, the AppStream release list and `distribumpy --version`
...
PACKAGE_WORK_ROOT="${DISTRIBUMPY_WORK_DIR:-$PACKAGING_DIR/out/build}"
...
# about this project that surprises everybody who packages it. `lxb-app` is a
...
require_toolkit_sources() {
    ...
    [[ -d "$PACKAGE_TOOLKIT_CRATES/lxb-app" ]] || package_die \
        "the toolkit's crate sources are not at $PACKAGE_TOOLKIT_CRATES/lxb-app.
...
package_toolkit_requirement() {
    ...
    [[ -n "$version" ]] || package_die "could not read the lxb-app requirement from Cargo.toml"
    printf '%s\n' "$version"
}
...
# A relative DISTRIBUMPY_WORK_DIR has to become absolute here rather than at
...
if [[ -n "${DISTRIBUMPY_WORK_DIR:-}" ]]; then
    package_set_work_root "$DISTRIBUMPY_WORK_DIR"
fi
...
Point the build somewhere with room using --work-dir DIR (or DISTRIBUMPY_WORK_DIR),
