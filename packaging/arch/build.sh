...
Examples:
  ...
  packaging/arch/build.sh --work-dir /var/tmp/distribumpy
EOF
}
...
work="$(package_work_dir distribumpy-arch)"
cleanup() {
    if [[ -n "${work:-}" && "$work" == */distribumpy-arch.* && -d "$work" ]]; then
        rm -rf -- "$work"
    fi
}
trap cleanup EXIT
...
source_archive="$work/distribumpy-$PACKAGE_VERSION.tar.gz"
